...
[简体中文](README.md) · [Agent entrypoint](SKILL.en.md) · [Companion notification guide](https://github.com/IIMovoMII/codex-desktop-notification-field-guide)
...
| --- | --- |
| [Discovery](references/discovery.en.md) | Inventory Codex, configuration, credentials, history and endpoint capabilities |
| [Architecture](references/architecture.en.md) | Define profile ownership, first-use flow and transactions |
| [First use and restart checkpoints](references/first-use-bootstrap.en.md) | Initialize a missing official or API state across the required Codex restarts |
| [Config recovery and external writers](references/config-recovery.en.md) | Handle CC Switch ownership conflicts, empty/template configs and offline repair |
| [Optional multi-relay profiles](references/multi-relay-profiles.en.md) | Build official-account and multi-relay switching with profile-specific models and credentials |
| [History compatibility](references/history-compatibility.en.md) | Keep local conversations resumable across auth modes |
| [Network diagnostics](references/network-diagnostics.en.md) | Separate HTTPS, WebSocket, relay and system-proxy failures |
| [Safety and rollback](references/safety-and-rollback.en.md) | Protect credentials and recover from interrupted writes |
| [Validation](references/validation.en.md) | Build a test matrix before trusting the switcher |
