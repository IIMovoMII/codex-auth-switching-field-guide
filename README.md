...
> 这里写的是目标、踩坑经验和验收方法，不是唯一源码。智能体应结合本机系统和用户需求，自由替换实现方式、增删功能与调整界面；仓库中的 Windows 路径和做法只是已验证过的参考。
...
智能体能从电脑读取的内容，例如操作系统、Codex 版本、文件位置和现有配置，不应再拿来问用户。只需要询问无法自行确定、而且会改变产品结果的选择，例如：要管理多少官方账号和中转站、是否处理旧对话、想用命令行还是小应用、是否接受切换时关闭 Codex、哪些代理和模型需要随档案切换。

用户可以只做“两种模式一键切换”，也可以增加多账号、多中转、历史修复、状态页或自动诊断；不需要的模块可以删掉。安全写入、凭据隔离、无关配置保留和可回滚，是实现形式变化后仍应保留的底线。
...
让智能体阅读 [SKILL.md](SKILL.md)，并要求它针对当前电脑设计方案。智能体应先自动读取能够读取的环境信息，再集中询问账号数量、目标中转、界面形式、历史范围和停机偏好等真正需要用户决定的事项。一个合格的执行过程应当：
...
这里故意不提供可以盲装的认证管理程序。不同电脑的路径、系统、Codex 版本、中转能力、账号数量和风险承受范围差异很大，让每个人根据实际环境生成自己的实现更稳妥。

## 适用范围

本指南的方法可用于 Codex Desktop 和 Codex CLI 所在的不同系统；现有实战证据主要来自 Windows。其他平台不需要照抄 Windows 路径、进程关闭或凭据保护方式，智能体应使用该系统的等价机制并重新验证。仓库追求的是跨平台可适配，不是假装所有平台已经用同一份代码实测过。
