...
~~~text
Codex，请阅读 https://github.com/IIMovoMII/codex-auth-switching-field-guide，先只读检查我这台 Windows 电脑上的 Codex 版本、实际生效的配置、当前认证类型、本地历史存储、网络路线，以及 CC Switch 等可能改写 `config.toml` 的外部管理器；先让我选择唯一配置写入者，并提醒我在第一次修改前自行保存一份经过真实请求验证的完整 `config.toml`，不要持续生成无限备份。再根据本机实际情况为我设计并构建一套可回滚的官方 OAuth 账号／API 兼容供应商切换器，明确处理本机从未有过官方登录态、从未配置过中转／API 地址和密钥、以及配置被清空或改成通用模板后的离线恢复；Codex 已无法对话时，要允许我用本地 PowerShell 或另一个编程智能体恢复。密钥只允许通过本地安全入口输入而不能发进对话，保留实时配置中的插件、MCP、Skills、钩子、权限和项目设置；根据我的选择支持多个中转档案，把每个中转站的地址、受保护凭据、模型与代理策略分别绑定到档案，让我可以完全不用 CC Switch 切换中转站，但不要把整份 `config.toml` 保存进每个档案。只有在当前 Codex 版本实测通过后，才让官方和 API 档案都固定使用 `model_provider = "openai"`。把历史兼容准备做成独立操作：扫描所有当前及归档的本地会话 JSONL 和相关 SQLite，脱敏统计每种 provider；经我确认且 Codex 完全退出后，用经过验证的备份和字段级回滚清单，把所有语义等价的本地 provider 元数据统一为 `openai`，不要修改云端 ChatGPT Work／Chat 会话，把响应项目编号修复作为另一条独立规则，并保证以后新建会话也写入 `openai`。首次初始化要做成可跨 Codex 完全重启继续的持久检查点流程，因为部分配置只有新进程才能验证；修改当前认证或对话历史前先向我说明影响并获得确认，最后逐阶段验证每次重启后的真实状态、各档案切换、故障回滚、网络行为、新会话元数据和代表性旧对话在两种模式下的续聊。
~~~
...
官方模式不保留 `openai_base_url`；API 兼容档案则写入这个顶层地址覆盖，并启用自己的凭据。当前 [Codex 配置参考](https://developers.openai.com/codex/config-reference) 把 `openai` 列为内置默认 provider，也把 `openai_base_url` 定义为它的接口地址覆盖。不要再用 `[model_providers.openai]` 表去重新定义这个保留名称。
