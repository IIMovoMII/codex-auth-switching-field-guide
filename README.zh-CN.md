...
~~~text
Codex，请阅读 https://github.com/IIMovoMII/codex-auth-switching-field-guide，先只读检查我这台 Windows 电脑上的 Codex 版本、实际生效的配置、当前认证类型、本地历史存储和网络路线，再根据本机实际情况为我设计并构建一套可回滚的官方 OAuth 账号／API 兼容供应商切换器，明确处理本机从未有过官方登录态以及从未配置过中转／API 地址和密钥的情况，密钥只允许通过本地安全入口输入而不能发进对话，保留实时配置中的插件、MCP、Skills、钩子、权限和项目设置，支持不同供应商各自的模型与代理策略，并且只有在当前 Codex 版本实测通过后，才让官方和 API 档案都固定使用 `model_provider = "openai"`。把历史兼容准备做成独立操作：扫描所有当前及归档的本地会话 JSONL 和相关 SQLite，脱敏统计每种 provider；经我确认且 Codex 完全退出后，用经过验证的备份和字段级回滚清单，把所有语义等价的本地 provider 元数据统一为 `openai`，不要修改云端 ChatGPT Work／Chat 会话，把响应项目编号修复作为另一条独立规则，并保证以后新建会话也写入 `openai`。首次初始化要做成可跨 Codex 完全重启继续的持久检查点流程，因为部分配置只有新进程才能验证；修改当前认证或对话历史前先向我说明影响并获得确认，最后逐阶段验证每次重启后的真实状态、各档案切换、故障回滚、网络行为、新会话元数据和代表性旧对话在两种模式下的续聊。
~~~
